...
async def _render_single_sample_robust(
        ...
        async with sem:
            try:
                ...
                if res.is_successful:
                    ...
                    resized = resize_spatial_dimensions(
                        ImageTensor(raw_tensor=t), image_size, image_size
                    )
    ...
    if not success:
        raise RuntimeError(
            f"Failed to generate a valid non-degenerate sample for family "
            f"'{family}' after {max_retries} attempts."
        )
...
async def _render_stage_batch_async(
    ...
    """Compile markups with bounded concurrency, retries, and manifest metrics."""
    ...
    while i < total:
        ...
        logger.log(
            f"  -> Rendered [{chunk_end}/{total}] images in {elapsed:.1f}s ({rate:.1f} img/s)..."
        )
        i = chunk_end
...
def build_stage_dataset(
    ...
    train_cache: Path | None = (
        cache_dir / f"{cache_tag}_train.pt" if cache_dir is not None else None
    )
    ...
    sample_fams, sample_seeds = generate_stage_seeds(families, num_samples, base_seed=seed)
    ...
    if cache_dir is not None and train_cache is not None and val_cache is not None:
        ...
        torch.save(
            {"images": images_tensor[train_idx], "tokens": encoded_tokens[train_idx]}, train_cache
        )
...
def run_curriculum_v3(args: argparse.Namespace) -> None:
    ...
    log_path: Path = (
        ...
        else results_dir / "train_v3_phase1_retrained.log"
    )
    ...
    if vocab_path.exists():
        ...
        logger.log(
            f"[+] Loaded vocabulary: {len(vocabulary.token_to_index)} tokens "
            f"(SHA-256: {vocab_hash})"
        )
    else:
        ...
        logger.log(
            f"[+] Built and saved vocabulary: {len(vocabulary.token_to_index)} tokens "
            f"(SHA-256: {vocab_hash})"
        )
    ...
    logger.log(
        f"[+] Model Architecture: {total_params:,} parameters (CoordConv + 2D PE + Dropout 0.1)"
    )
    ...
    logger.log(
        f"[*] Complete experiment configuration dumped to {results_dir / 'experiment_config.json'}"
    )
    ...
    for stage_idx, stage_cfg in enumerate(
        CURRICULUM_STAGES[args.start_stage - 1 :], start=args.start_stage
    ):
        ...
        logger.log(
            f"    Effective Batch: {args.micro_batch_size} x {accumulation_steps} = {eff_bs}"
        )
        logger.log("=" * 72)
        ...
        logger.log(
            f"[+] Stage {stage_idx} Data Shape: Train {train_images.shape} | Val {val_images.shape}"
        )
        ...
        for epoch in range(1, stage_epochs + 1):
            ...
            if val_loss < stage_best_val:
                ...
                saved_marker = (
                    f"-> Saved [Stage {stage_idx} Best: {stage_ckpt_path.name} | "
                    f"SHA-256: {ckpt_hash}]"
                )
            ...
            if val_loss < best_global_loss:
                ...
                saved_marker += (
                    f" -> [GLOBAL BEST: {global_ckpt_path.name} | SHA-256: {global_hash}]"
                )
    ...
    logger.log(
        ...
        f"{compute_file_sha256(checkpoints_dir / 'curriculum_v3_best.pt')}"
    )
    logger.log("=" * 72)
